(function () {
  ...
  function makeCocktail(parent, dispatcher, mun, data) {
    ...
    parent.append('p').html('<strong>Población:</strong> ' + (+mun.properties.population).toLocaleString('es-BO')); // map1Number should always be present - NaN if no tests
  ...
  function makeLimits(parent, dispatcher, mun, data) {
    parent.html(null);
    makeHeader(parent, mun.properties.name, mun.properties.fuName);
    parent.append('p').html('<strong>Población:</strong> ' + (+mun.properties.population).toLocaleString('es-BO')); // map2Category should always be present
