(function () {
  ...
  function loadData(dispatcher) {
    const promises = [
      ...
      csv$1(cfg.substances.url, {integrity: cfg.substances.integrityHash}),
      json(cfg.topojson.url, {integrity: cfg.topojson.integrityHash}),
      csv$1(cfg.values.url, {integrity: cfg.values.integrityHash}, row => {
        return {
          category: {
  ...
  function makeTubesCocktail(parent, substances, titleHtml, tubeClass) {
    ...
    text
      ...
      .attr('x', '0')
      .attr('dy', '0')
      .text(sub => sub.numDetections + ' detecções');
    text
      .append('tspan')
      .attr('x', '0')
      .attr('dy', '1.2em')
  ...
  function makeDetails(parent, dispatcher, view, initState) {
    ...
    dispatcher.on('to-mun-view.details', mun => {
      makeMun$1(parent, dispatcher, view, {data: initState.data, mun: mun});
    });
  ...
  function makeBrazil$1(parent, dispatcher, view, state) {
    ...
    if (view === 'limits') {
      ...
      main
        .append('p')
        .classed('brazil', true)
        .html('Agrotóxicos foram detectados na água que abastece mais de 2.300 cidades de 2014 a 2017. Clique no mapa ou digite sua cidade para descobrir quais produtos químicos saíram da sua torneira em concentração acima do limite considerado seguro.');
      ...
      main
        .append('p')
        .classed('brazil', true)
        .html('Agrotóxicos foram detectados na água que abastece mais de 2.300 cidades de 2014 a 2017. Clique no mapa ou digite sua cidade para descobrir quais produtos químicos saíram da sua torneira.');
  ...
  function makeCocktail(parent, dispatcher, mun, data) {
    ...
    if (isNaN(mun.properties.map1Number)) {
      ...
      parent
        .append('header')
        .html(
          '<span class="is-size-4">Sem dados</span>' +
            ' sobre agrotóxicos na água que abastece ' +
            mun.properties.name +
            '.'
        );
      ...
      parent
        .append('header')
        .html(
          '<span class="is-size-4">Nenhum agrotóxico</span>' +
            ' detectado na água que abastece ' +
            mun.properties.name +
            '.'
        );
    } else {
      ...
      parent
        ...
        .html(
          '<span class="is-size-4">' +
            mun.properties.map1Number +
            '</span> agrotóxico(s)' +
            ' detectado(s) na água que abastece ' +
            mun.properties.name +
            '.'
        );
      ...
      if (hhceSubstances.length > 0) {
        makeTubesCocktail(
          ...
          '<span class="is-size-4">' +
            ...
            'como câncer, defeitos congênitos e distúrbios endócrinos',
          'hhce'
        );
  ...
  function makeLimits(parent, dispatcher, mun, data) {
    ...
    if (mun.properties.map2Category === MAP2.CATEGORY.NO_TEST) {
      parent
        ...
        .html(
          '<span class="is-size-4">' +
            'Sem dados</span> sobre agrotóxicos na água que abastece ' +
            mun.properties.name +
            '.'
        );
      ...
      parent
        ...
        .html(
          '<span class="is-size-4">' +
            'Nenhum </span> agrotóxico detectado acima dos limites brasileiros ou europeus em ' +
            mun.properties.name +
            '.'
        );
    } else {
      ...
      if (supBrSubstances.length > 0) {
        makeTubesLimits(
          ...
          '<span class="is-size-4">' +
            ...
            '</span> agrotóxico(s) detectado(s) na água em concentração acima do limite considerado seguro no Brasil em ' +
            mun.properties.name,
      ...
      if (supEuSubstances.length > 0) {
        makeTubesLimits(
          ...
          '<span class="is-size-4">' +
            ...
            '</span> agrotóxico(s) detectado(s) acima do limite considerado seguro na União Europeia',
  ...
  */
  ...
  function makeCocktailToOtherViews(parent, dispatcher, state) {
    ...
    par
      .append('a')
      .attr('href', '#')
      .text('detectadas acima dos limites legais na água' + suffix)
      .on('click', () => {
  ...
  function makeLimitsToOtherViews(parent, dispatcher, state) {
    ...
    par
      .append('a')
      .attr('href', '#')
      .text('total de substâncias detectadas' + suffix)
      .on('click', () => {
  ...
  function makeToArticle(parent) {
    const par = parent
      ...
      .html(null)
      .append('p');
    par.append('span').text('Entenda: leia ');
    par
      .append('a')
      .attr(
        'href',
  ...
  function makeSource(parent) {
    ...
    li2
      ...
      .attr('target', '_top')
      .text('IBGE 2017');
    li2.append('span').text(' (FTP). Instituto Brasileiro de Geografia e Estatística (IBGE), 2017.');
  }
  ...
  const cocktailColor = function(value) {
    /*if (isNaN(value)) {
  ...
  const limitsColor = function(value) {
  ...
  const cfg$2 = {
    ...
    limits: {
      ...
      labels: {
        BELOW: 'Todos os agrotóxicos dentro dos limites',
        NO_TEST: 'Sem dados',
  ...
  function createLegendCocktail(parent) {
    ...
    label
      ...
      .text('(claro: nenhum, escuro: 27 agrotóxicos diferentes)');
  ...
  */
  function createLegendCocktail(parent) {
    ...
    const yy = linear$1()
      .domain([0, cfg$2.cocktail.keys.length])
      .rangeRound([
        0,
        (cfg$2.cocktail.dim.height + cfg$2.cocktail.dim.margin) *
          cfg$2.cocktail.keys.length,
      ]);
    ...
    legend
      .selectAll('rect')
      .data(cfg$2.cocktail.keys)
      .enter()
      .append('rect')
      .attr('fill', key => cocktailColor(MAP1.CATEGORY[key]))
      .attr('height', cfg$2.cocktail.dim.height)
      .attr('x', 0)
    ...
    legend
      ...
      .enter()
      .append('text')
      .attr('x', cfg$2.cocktail.dim.label.xOffset)
      .attr('y', (key, idx) => yy(idx) + cfg$2.cocktail.dim.label.yOffset)
      .attr('font-size', '0.9rem')
      .text(key => cfg$2.cocktail.labels[key]);
    ...
    label
      .append('text')
      .attr('y', -cfg$2.cocktail.dim.titleOffset)
      .attr('font-weight', 'bold')
      .text('Número de agrotóxicos');
    label
      .append('text')
      .attr('y', -cfg$2.cocktail.dim.subtitleOffset)
      .attr('font-weight', 'bold')
      .text('detectados na água');
  }
  ...
  function createLegendLimits(parent) {
    ...
    const yy = linear$1()
      .domain([0, cfg$2.limits.keys.length])
      .rangeRound([
        0,
        (cfg$2.limits.dim.height + cfg$2.limits.dim.margin) * cfg$2.limits.keys.length,
      ]);
    ...
    legend
      .selectAll('rect')
      .data(cfg$2.limits.keys)
      .enter()
      .append('rect')
      .attr('fill', key => limitsColor(MAP2.CATEGORY[key]))
      .attr('height', cfg$2.limits.dim.height)
      .attr('y', (key, idx) => yy(idx))
    ...
    legend
      ...
      .enter()
      .append('text')
      .attr('x', cfg$2.limits.dim.label.xOffset)
      .attr('y', (key, idx) => yy(idx) + cfg$2.limits.dim.label.yOffset)
      .attr('font-size', '0.9rem')
      .text(key => cfg$2.limits.labels[key]);
    ...
    label
      .append('text')
      .attr('y', -cfg$2.limits.dim.titleOffset)
      .attr('font-weight', 'bold')
      .text('Concentração dos agrotóxicos na água');
    label
      .append('text')
      .attr('y', -cfg$2.limits.dim.subtitleOffset)
      .attr('font-weight', 'bold')
      .text('acima dos limites considerados seguros');
  }
  ...
  const map2LabelByCategory = [
    'Nunca testado',
    'Todas as substâncias abaixo dos limites brasileiro e europeu',
    'Agrotóxico(s) detectado(s) acima do limite europeu',
    'Agrotóxico(s) detectado(s) acima do limite brasileiro',
  ];
  function cocktailLabel(mun) {
    return Number.isInteger(mun.properties.map1Number)
      ? mun.properties.map1Number + ' agrotóxico(s) encontrado(s) na água potável.'
      : 'Nunca testado.';
  }
  ...
  function makeMap(parent, dispatcher, data) {
    ...
    canvas
      .attr('width', cfg$4.drawing.scale * cfg$4.data.width)
      .attr('height', cfg$4.drawing.scale * cfg$4.data.height);
    const context = canvas.node().getContext('2d');
    const scale = cfg$4.drawing.scale;
    ...
    const svg = parent
      ...
      .style('top', '0px')
      .style('left', '0px')
      .attr('viewBox', '0,0,' + cfg$4.data.width + ',' + cfg$4.data.height);
