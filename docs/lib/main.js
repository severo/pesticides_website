(function () {
  ...
  function create(state, content) {
    ...
    createCountries(map, path, state.data.geojson[level].countries, cfg.countries);
  ...
  function appendContent(dispatcher, parent) {
    var content = parent.append('div').attr('id', cfg$1.id);
    startLoading(content);
    dispatcher.on('state-changed.content', function (state) {
      startLoading(content);
      create$1[state.zoom][state.view](state, content);
      endLoading(content);
    });
    return content;
  }
  ...
  function append(dispatcher, parent) {
    var control = parent.append('div').attr('id', cfg$2.id).classed(cfg$2.class, true);
    var ul = control.append('ul');
    var li = ul.selectAll('li').data(cfg$2.options).enter().append('li').attr('id', function (opt) {
      return opt.id;
    });
    ...
    li.on('click', function (data, id, cur) {
      // change style
      li.classed(cfg$2.isActiveClass, false); // TODO: does not seem optimal, surely we can have access to the element

      select(cur[id]).classed(cfg$2.isActiveClass, true); // invoke callbacks

      dispatcher.call(cfg$2.callbackTypename, null, {
        selected: data.id
      });
    });
    return control;
  ...
  function append$1(dispatcher, parent) {
    var control = parent.append('nav').attr('id', cfg$3.id).classed(cfg$3.class, true);
    var ul = control.append('ul');
    ...
    li.on('click', function (data, id, cur) {
      // change style
      li.classed(cfg$3.isActiveClass, false); // TODO: does not seem optimal, surely we can have access to the element

      select(cur[id]).classed(cfg$3.isActiveClass, true); // invoke callbacks
  ...
  function loadData() {
    ...
    var keys = Object.keys(cfg$4);
    var promises = keys.map(function (key) {
      var datasetCfg = cfg$4[key];
